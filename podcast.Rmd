...
```{r read-file}
# Import data
train_pod <- read_csv("C:/Users/Anthony Ebuboh/Downloads/podcast_train (1).csv") %>%
  clean_names() %>%
  select(-id)

str(train_pod)

test_pod <- read_csv("C:/Users/Anthony Ebuboh/Downloads/podcast_test (2).csv") %>%
  clean_names() %>%
  select(-id)
...
submit <- read_csv("C:/Users/Anthony Ebuboh/Downloads/podcast_submission (1).csv")
...
## Data splitting and fold creation
```{r}
# Data Splitting
set.seed(1957)
...
## Define,tune and fit final random forest model
```{r}
